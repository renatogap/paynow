class PesquisarAcaoController {
    ...
    criar(e) {
        window.location = `${e.dataset.url}/seguranca/acao/novo`;
    }
    editar(e) {
        if (oTable.rows('.selected').data()[0] != null) {
            window.location = `${e.dataset.url}/seguranca/acao/editar/` + oTable.rows('.selected').data()[0].id;
        } else
